...
from memory_inference.memory.policies import mem0_validity_guard_policy
...
def test_mem0_validity_guard_suppresses_stale_support_for_current_query() -> None:
    policy = mem0_validity_guard_policy(
    ...
    result = policy.retrieve_for_query(
        make_query(
            ...
            entity="Alice",
            attribute="employer",
            question="Does Alice still work at Google?",
            answer="Meta",
            timestamp=3,
            session_id="s",
    ...
    assert DeterministicValidityReader().answer(
        make_query(
            query_id="q-current",
            entity="Alice",
            attribute="employer",
...
def test_mem0_validity_guard_surfaces_unresolved_conflicts_for_abstention() -> None:
    policy = mem0_validity_guard_policy(
        consolidator=MockConsolidator(),
        encoder=KeywordDenseEncoder(),
    )
    ...
    result = policy.retrieve_for_query(query)

    assert result.debug["retrieval_mode"] == "mem0_validity_conflict_guard"
    assert {entry.value for entry in result.entries if entry.attribute == "employer"} == {"Google", "Meta"}
    assert DeterministicValidityReader().answer(query, result.entries) == "ABSTAIN"
